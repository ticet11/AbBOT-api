...
for zipObj in list(zips["texaszips"]):
    ...
    if i > 0:
        weight = weight + zipWeights[i - 1]
    zipWeights.append(weight)
    i = i + 1
...
gop_members = [
    ...
    'Cole Hefner', 'Matt Schaefer', 'Jay Dean', 'Cody Harris', 'Chris Paddie',
    'Travis Clardy', 'Kyle Kacal', 'Ben Leman', 'John N. Raney', 'Steve Toth',
    'Will Metcalf', 'John Cyrier', 'Ernest Bailes', 'James White',
    'Terry Wilson', 'Dade Phelan', 'Mayes Middleton', 'Greg Bonnen',
    'Cody Vasut', 'Brooks Landgraf', 'Tom Craddick', 'Dustin Burrows',
    'John Frullo', 'Phil Stephenson', 'John T. Smithee', 'Four Price',
    'Ken King', 'Candy Noble', 'Stephanie Klick', 'Jeff Cason', 'Matt Krause',
    'Tony Tinderholt', 'David Cook', 'Craig Goldman', 'Giovanni Capriglione',
    'Charlie Geren', 'Sam Harless', 'Dan Huberty', 'Briscoe Cain',
    'Dennis Paul', 'Tom Oliverson', 'Mike Schofield'
]
firstNames = [
    'Hannah', 'Olivia', 'Marcia', 'Sarah', 'Tara', 'Brooke', 'Wanda', 'Andrea',
    'Julie'
]
lastNames = [
    'Morgan', 'Walker', 'Lewis', 'Butler', 'Jones', 'Barnes', 'Martin',
    'Wright', 'Foster'
]
...
def generate_text(prompt_text: str,
    ...
    seq_length = adjust_seq_length_to_model(
        seq_length, max_sequence_length=model.config.max_position_embeddings)
...
def create_anonymous_form_batch(prompt_text='Dear Gov. Abbott,', batch_size=5):
    ...
    text_sequences = generate_text(prompt_text,
                                   num_return_sequences=batch_size)

    form_batch = []

    for i in range(batch_size):
        zipKey = random.choices(list(zips["texaszips"]),
                                cum_weights=zipWeights,
                                k=1)[0]
        ...
        userLocation = {
            ...
            'userState':
            zipObj["statename"]
            if random.choice([True, False]) else zipObj["stateid"],
            'userZip':
            zipKey,
        }
        form_data = {
